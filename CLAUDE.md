...
- **index.html**: Title input, an empty `#input-rows` container (input rows are generated by JS), action buttons (Chronologize, Clear All), and SVG container (with an `<h2>` heading above the SVG for the title)
...
**Multi-Row Input**:
- Input rows (start date, end date, label, +/− buttons) are created dynamically by `addInputRow()`; the first row is added in the constructor
- `+` inserts a new row below the clicked row; `−` removes its row and is disabled when only one row remains
